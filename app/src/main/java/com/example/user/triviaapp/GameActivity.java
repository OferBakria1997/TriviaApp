...
public class GameActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        HttpTriviaGet getRequest = new HttpTriviaGet(this);
        try {
            String result = getRequest.execute(baseUrl).get();
            thread.join(2000);//to die the thread
            ...
            if (result.equals(response)) {
                Intent intentt = new Intent(GameActivity.this, MainActivity.class);
    ...
    public void responseRecieved() {
        ...
        for (int i = 0; i < retrievedQuestionsJSON.length(); i++) {
            ...
            try {
                ...
                if (que.matches("multiple")) {
                    Question newQuestion = new Question(currentObj.getString("category"), currentObj.getString("type"), currentObj.getString("difficulty"), currentObj.getString("question"), currentObj.getString("correct_answer"), currentObj.getJSONArray("incorrect_answers").getString(0), currentObj.getJSONArray("incorrect_answers").getString(1), currentObj.getJSONArray("incorrect_answers").getString(2));
                    questionsList.add(newQuestion);
                    Log.d("result object: ", newQuestion.toString());
                } else {
                    Toast.makeText(getApplicationContext(), "yofeeeee", Toast.LENGTH_SHORT).show();
                }
            } catch (JSONException e) {
                e.printStackTrace();
                continue;
    ...
    public void refreshList(int index) {
        ...
        if (random % 2 == 0)
            answers = new String[]{questionsList.get(index).incorrect_answer3, questionsList.get(index).correct_answer, questionsList.get(index).incorrect_answer1, questionsList.get(index).incorrect_answer2};
        else if (random % 3 == 0)
            answers = new String[]{questionsList.get(index).incorrect_answer2, questionsList.get(index).correct_answer, questionsList.get(index).incorrect_answer1, questionsList.get(index).incorrect_answer2, questionsList.get(index).incorrect_answer3};
        else
            answers = new String[]{questionsList.get(index).correct_answer, questionsList.get(index).incorrect_answer1, questionsList.get(index).incorrect_answer2, questionsList.get(index).incorrect_answer3};
